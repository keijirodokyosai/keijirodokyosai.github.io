...
| 背景 PNG | `pdf/soshiki-form-enter.pdf` から生成。㊞除去済み。組合員欄の郵便番号用 **背景ハイフン** 除去済み（§9.7）。性別欄の **1.男 / 2.女** 印刷除去済み（§9.7.1） |
...
6. 組合員欄 CSS の最終調整（住所5枠の横位置など）・開発用仮表示の削除（郵便番号枠は §9.8 完了済み）
7. 組合名プルダウン（localStorage）・追加確認・削除 UI
8. `validateSoshikiForm()` の配線 → 確認画面・PDF 出力・メール送信（任意・後回し可）
